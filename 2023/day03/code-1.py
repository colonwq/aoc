...
def find_numbers_2(lines = [], j = 0, i = 0):
  number_prod = number = 0
  numbers = []
  ...
  if j > 0:
    ...
    for iup in range( max(0,i-1), min(i+2, len(lines[jup]) ) ):
      if lines[jup][iup].isdigit():
        ...
        number = build_number(lines,jup,iup)
        if number > 0:
  ...
  if j < len(lines)-1:
    ...
    for idown in range( max(0,i-1), min(i+2, len(lines[jdown]) ) ):
      if lines[jdown][idown].isdigit():
        ...
        number = build_number(lines,jdown,idown)
        if number > 0:
          numbers.append( number )
  #search i-1
  if i > 0:
    ...
    if lines[j][ileft].isdigit():
      ...
      number = build_number(lines,j,ileft)
      if number > 0:
        numbers.append( number )
  #search i+1
  if i < len(lines[j])-1:
    ...
    if lines[j][iright].isdigit():
      ...
      number = build_number(lines,j,iright)
      if number > 0:
        numbers.append( number )

  #print("Number sum: %d" %(number_sum) )
  if len(numbers) == 2:
    number_prod = numbers[0] * numbers[1]
    #print("The gear ratio is (%d * %d) %d" %(numbers[0],numbers[1], number_prod))
  return number_prod
...
def process_lines_2( lines = [] ):
  ...
  while j < len(lines):
    while i < len(lines[0]):
      if lines[j][i] == "*":
        #print("Found a part at [%d %d] %s" %(j,i,lines[j][i]))
        #print(lines[j-1])
        #print(lines[j])
        #print(lines[j+1])
        retval += find_numbers_2(lines, j, i)
...
#    print("Answer 1 %d" % (answer) )
    ...
    answer = process_lines_2(lines)

    print("Too high: 82860869")
    print("Answer 2: %d" % (answer) )
